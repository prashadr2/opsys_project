...
bool preemption_arrivalSRT(Process* incpu, std::list<Process>& ready, int t, int tcs);
...
bool preemption_ioSRT(Process*& incpu, std::list<Process>& ready, std::list<Process>& waiting, int& t, int tcs, auto sortfunc){ //call when finish IO and ready.push_back
    ...
    waiting.pop_front();
    // Process tmprdyback(ready.back());
    // Process tmpincpu(*incpu);
    ...
    // ready.pop_back();
    // ready.push_back(*incpu);
    // incpu = new Process(tmprdyback);;
    // std::cout << "time " << t << "ms: Process " << ready.back().getname() << " (tau " << ready.back().getTau() << "ms) completed I/O; preempting " << incpu->getname() << ' ';
    // printqueueSRT(ready);
    t += tcs/2;
...
bool preemption_arrivalSRT(Process* incpu, std::list<Process>& ready, int t, int tcs){ //call when we have an arrival and ready.push_back
    //ready.back() is the process to preempt...
    if(ready.back().getcurrentruntime() >= incpu->getcurrentruntime()) return false;
